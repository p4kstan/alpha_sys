...
async function buildGoogleConversion(
  ...
  if (!gclid && !gbraid && !wbraid && sessionId) {
    ...
    if (sess?.gclid || sess?.gbraid || sess?.wbraid) {
      gclid  = gclid  || sanitizeClickId(sess.gclid);
      gbraid = gbraid || sanitizeClickId(sess.gbraid);
      wbraid = wbraid || sanitizeClickId(sess.wbraid);
      console.log(`[google-ads-capi] fallback session HIT session_id=${sessionId} gclid=${gclid||"-"} gbraid=${gbraid||"-"} wbraid=${wbraid||"-"}`);
    } else if (sessionId) {
