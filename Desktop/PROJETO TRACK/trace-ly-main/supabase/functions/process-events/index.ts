...
// Auto-process queue every 60 seconds — no manual trigger needed.
Deno.cron("process-event-queue", "* * * * *", async () => {
  try {
    ...
    const url = `${Deno.env.get("SUPABASE_URL")}/functions/v1/process-events`;
    await fetch(url, {
      method: "POST",
      headers: {
    ...
    console.error("cron process-event-queue error:", e);
  }
});
