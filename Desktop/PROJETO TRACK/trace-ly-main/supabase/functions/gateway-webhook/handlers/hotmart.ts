...
import { dig, extractTrackingFromMetadata, num, str } from "./_helpers.ts";
...
export const hotmartHandler: GatewayHandler = {
  ...
  normalize: (p) => {
    ...
    // Hotmart forwards SDK metadata (gclid, fbp, utm_*, event_id) via
    // purchase.tracking_parameters when the checkout URL was decorated.
    const meta = purchase.tracking_parameters || purchase.metadata || {};
