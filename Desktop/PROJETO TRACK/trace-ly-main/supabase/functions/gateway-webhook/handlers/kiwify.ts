...
import { extractTrackingFromMetadata, num, str } from "./_helpers.ts";
...
export const kiwifyHandler: GatewayHandler = {
  ...
  normalize: (p) => {
    ...
    // Kiwify forwards SDK metadata (gclid, fbp, utm_*, event_id) via
    // trackingParameters when the checkout URL was decorated.
    const meta = p.trackingParameters || p.tracking_parameters || p.metadata || {};
